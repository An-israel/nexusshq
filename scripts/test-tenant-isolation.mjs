...
async function makeWorkspace(slug, ownerId) {
  const { data: ws, error } = await admin
    .from("workspaces")
    .insert({ name: slug, slug, plan: "business" })
    .select("id, slug")
    .single();
  if (error) throw error;
  ...
  await admin
    .from("profiles")
    .upsert({ id: ownerId, full_name: slug, email: `iso-${slug}@example.test` });
  await admin
    .from("workspace_members")
    .insert({ workspace_id: ws.id, user_id: ownerId, role: "owner", is_active: true });
  return ws;
}
...
try {
  ...
  const { data: chA } = await admin
    .from("channels")
    .insert({ workspace_id: wsA.id, name: "general-a", type: "public", created_by: userA.id })
    .select()
    .single();
  const { data: chB } = await admin
    .from("channels")
    .insert({ workspace_id: wsB.id, name: "general-b", type: "public", created_by: userB.id })
    .select()
    .single();
  ...
  if ((wsListA ?? []).some((w) => w.id === wsB.id)) fail("User A can see workspace B");
  else ok("Workspace isolation A");
  const { data: wsListB } = await cB.from("workspaces").select("id");
  if ((wsListB ?? []).some((w) => w.id === wsA.id)) fail("User B can see workspace A");
  else ok("Workspace isolation B");

  // 2. Channels
  const { data: chListA } = await cA.from("channels").select("id");
  if ((chListA ?? []).some((c) => c.id === chB.id)) fail("User A can see channel B");
  else ok("Channel isolation A");

  // 3. Cross-tenant write rejection
  const { error: insErr } = await cA
    .from("messages")
    .insert({ workspace_id: wsB.id, sender_id: userA.id, body: "leak", channel_id: chB.id });
  if (!insErr) fail("User A could insert message into workspace B");
  else ok("Cross-tenant message insert blocked");

  // 4. Member listing
  const { data: memList } = await cA.from("workspace_members").select("workspace_id");
  if ((memList ?? []).some((m) => m.workspace_id === wsB.id))
    fail("User A can see workspace B members");
  else ok("Membership isolation");
  ...
  if ((dmListA ?? []).some((d) => d.workspace_id === wsB.id))
    fail("User A can read workspace B DMs");
  else ok("DM read isolation");
  ...
  if (!dmInsErr) fail("User A could insert DM into workspace B");
  else ok("DM cross-tenant insert blocked");

  // 6. Group messages — seed in B, ensure A cannot read or write
  const { data: grpB } = await admin
    .from("message_groups")
    .insert({
      workspace_id: wsB.id,
      name: "grp-b",
  ...
  await admin
    ...
    .insert({ group_id: grpB.id, user_id: userB.id, workspace_id: wsB.id });
  ...
  if ((grpListA ?? []).some((g) => g.workspace_id === wsB.id))
    fail("User A can read workspace B group messages");
  else ok("Group message read isolation");
  ...
  if (!grpInsErr) fail("User A could insert group message into workspace B");
  else ok("Group message cross-tenant insert blocked");
  ...
  if ((stListA ?? []).some((s) => s.workspace_id === wsB.id))
    fail("User A can read workspace B standups");
  else ok("Standup read isolation");
  ...
  if (!stInsErr) fail("User A could insert standup for user B");
  else ok("Standup cross-user insert blocked");
  ...
  if ((invListA ?? []).some((i) => i.workspace_id === wsB.id))
    fail("User A can read workspace B invites");
  else ok("Invite read isolation");
  const { error: invInsErr } = await cA.from("workspace_invites").insert({
    workspace_id: wsB.id,
    email: `leak-${stamp}@example.test`,
    role: "employee",
    token: `tok-leak-${stamp}`,
    invited_by: userA.id,
    expires_at: new Date(Date.now() + 7 * 86400000).toISOString(),
  });
  if (!invInsErr) fail("User A could create invite into workspace B");
  else ok("Invite cross-tenant insert blocked");

  // 9. Standups history (positive) — user B sees their own latest entry in their workspace
  const { data: stOwn } = await cB
    .from("standups")
    .select("id, today, workspace_id")
    .eq("workspace_id", wsB.id)
    .order("date", { ascending: false })
    .limit(1);
  if (!stOwn?.length || stOwn[0].today !== "b-today")
    fail("User B cannot read their own latest standup");
